...
Fixed version — handles Unicode, emojis, and French characters properly
"""
...
class ZAMIPDF(FPDF):
    """Custom PDF class with ZAMI branding — Unicode safe"""
    ...
    def section_title(self, title):
        """Render a styled section title — without emojis"""
    ...
    def clean_text(self, text):
        ...
        if not text:
            return ""
        # Remove emojis and other non-ASCII chars that cause issues
        ...
        text = text.replace('🗺️', '[Map]')
        text = text.replace('🌡️', '[Temp]')
        text = text.replace('📨', '[Mail]')
        text = text.replace('⬇️', '[Download]')
        text = text.replace('⚡', '[AI]')
        text = text.replace('🛠️', '[Tool]')
